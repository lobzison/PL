...
fun eval_prog (e,env) =
	...
	(case List.find (fn (s2,v) => s=s2) env of
	  ...
	  | Shift(d1,d2,e) => let val v = eval_prog(e, env)
	  					  in
							case v of 
							  NoPoints => NoPoints
							| Point(x,y) => Point(x+d1,y+d2)
							| Line(m,b) => Line(m, b + d2 - m * d1)
							| VerticalLine(x) => VerticalLine(x+d1)
							| LineSegment(x1,y1,x2,y2) => LineSegment(x1+d1,y1+d2,x2+d1,y2+d2)
						  end
